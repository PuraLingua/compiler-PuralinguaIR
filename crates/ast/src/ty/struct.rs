...
use token::{BraceCloseChar, BracketOpenChar, Keyword, KwStruct, SpecialChar, TokenKind};
...
pub struct StructDef {
    ...
    #[custom({
		let TypeAttrAndIndex { attr, index } = stream.parse::<TypeAttrAndIndex<{ TypeSpecificAttrType::Struct }>>()?;
        let index = index.get_value(stream.source()).unwrap().try_into().unwrap();
	})]
    pub attr: TypeAttr,
    #[do_not_generate]
    pub index: u32,
    pub ch_bracket_close: BraceCloseChar,
...
impl IToAbstract for StructDef {
